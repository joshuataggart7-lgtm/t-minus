...
- W5 fidelity: the shared Board snapshot now carries the record-derived method voice across cockpit and briefing, separates factor and evidence empties, keeps sole-source evaluation explicit, and omits receipt counts when the record read fails. Advisory only; no clock, hold, phase, required-document, seed, form, or external-write behavior changed.
