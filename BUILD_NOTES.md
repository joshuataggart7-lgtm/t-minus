...
- Target date: `memo-draft.ts` `situationScaffold` changes `No target award date is recorded.` to `Target award date: Not yet determined. Update as soon as the award date is known.` using the existing forecast constants. `briefing-book.ts` appends that same sentence to the phase/clock sub-line only when `targetAwardDate` is absent; a recorded target leaves the sub-line unchanged, and the R4 countdown figure/mode line is unchanged. `email-drafts.ts` was the only other generator with equivalent missing-target wording; its `Target award date: not recorded on the file.` now uses the same approved sentence.
