...
- Estimate removal: unsuccessful postaward Word outputs no longer fall back to `estimated_value`; blank Part 15 contract value prints `Not recorded`, while the companion omits the amount and formats a recorded numeric value as whole US dollars. The unsuccessful-letter and postaward-conference fields open blank, the BPA and four pre-award fields say estimated value, local prior-action comparables say `target award date` and `estimated value`, and IGCE-sourced CLINs retain item/description/quantity/unit while price fields become null at the shared form context.
- Copy only: vehicle totals say estimated value; small-business launched-file counts no longer call the values awards; document, Files, and Work Queue displays identify bare `estimated_value` amounts as IGCE. Today and palette results show no `estimated_value` amount, and the file hero has no bare value chip, so they required no text change. The Executive featured record already says `Est. value`; already-labelled estimate/IGCE, threshold, form-field, CSV, and non-`estimated_value` amounts remain unchanged.
- PDF boundary: `pdfGlyphs` maps minus, comparison signs, arrows, ellipsis, nonbreaking space, and bullets only for PDF output. Flattened PDFs apply it before the existing sanitization while retaining the section sign; AcroForm text applies it before a WinAnsi allowlist. All official AcroForms route through `pdfText`; no direct `setText` bypass exists. Screen, XFA/XDP, and Word text are unchanged.
- Export headers: `renderPdf` can draw the supplied header line on every page without moving body, insignia, running head, or footer. NF 1858 Word/PDF and generic/HQ-layout/TER/SAM-notice Word/PDF exports receive it. This covers packet transmittal, MFR, market research, evaluation of quotations, commerciality, PNM on either NF 1858 or generic print blocks, SAM notice, and TER. Dedicated NASA OP Word masters remain byte-identical because their dedicated dispatch branches do not call the generic exporter; official AcroForms, flattened form PDFs, XFA/XDP, and RFP-cover Word remain on their existing form-specific paths.
- Posting language: past-solicitation drafts with no saved posting date now say `(posting date not recorded)`; a recorded date with a past competitive phase uses received tense. Before-solicitation and recorded sole-source wording remain unchanged.
- Accessibility: the command palette listbox contains only labelled groups and options, status/help text is a sibling live region, list wrappers are presentational, and selected rows use the existing primary left-border selection treatment. Reporting headers use `text-muted-foreground`. The blackout draft, office-invite draft, report-views table, and preview-rows table are keyboard-focusable and labelled as scroll regions.
- Guardrails: no database, query, mutation, seed, migration, state/readiness/countdown/metrics/forecast, `sf-forms`, small-business calculation, layout/token, or official master change. PNM and SAM notice letterhead/title behavior remains unchanged apart from the requested export header.

# IA Fix R4 corrective — September 25, 2026

- State leak (`src/lib/memo-draft.ts`, `competitionBasis`): `Awarded on a sole-source basis under …` now remains byte-identical only when `ctx.operational.readiness === "LAUNCHED"`; otherwise it reads `This acquisition is being conducted on a sole-source basis under …`. Everything after `basis under` is unchanged.
- Solicitation tense (`src/lib/memo-draft.ts`, `competitionBasis`): when the ordered operational phase is past the solicitation phase and no posting date is recorded, competitive `A combined synopsis/solicitation will be posted to SAM.gov; quotations will be recorded on receipt.` becomes `A combined synopsis/solicitation was posted to SAM.gov; quotations were received.` (or the existing recorded-count grammar), and sole-source `A notice of intent to sole source will be posted to SAM.gov.` becomes `A notice of intent to sole source was posted to SAM.gov.` Posted-date wording and missing/not-past signals retain their prior text.
- Target date: `memo-draft.ts` `situationScaffold` changes `No target award date is recorded.` to `Target award date: Not yet determined. Update as soon as the award date is known.` using the existing forecast constants. `briefing-book.ts` appends that same sentence to the phase/clock sub-line only when `targetAwardDate` is absent; a recorded target leaves the sub-line unchanged, and the R4 countdown figure/mode line is unchanged. `email-drafts.ts` changes `Target award date: not recorded on the file.` to the same approved sentence. The document route export header was the only other generated-output hit; its `no target award date` fallback now uses the same approved sentence. File/requester on-screen notices and the briefing-book R4 forecast mode line are display copy, not additional generator fallbacks, and remain unchanged as directed.
- State-wording audit left unchanged: `briefing-book.ts` launched/since-award mode is selected by the recorded operational countdown supplied by the file page; `fpds-filling-sheet.ts` gates award wording on `isLaunched`; `sf-forms.ts` gates its award date on an actual `award_date`; memorandum chronology reports recorded audit events and uses the operational phase; `situation-memo.ts` carries the derived readiness type only; document/form export contexts and headers already use derived operational state or actual award dates. Template names/titles/field labels and Administration/Closeout template definitions were left unchanged. Postaward notification letters, their `award_amount`/`contract_value` fields, option-exercise text, Closeout documents, and SF 26/OF 347 award blocks were left unchanged because they are inherently post-award. Regulatory text, citations, agency-name uses of “Administration,” and comments were not lifecycle assertions.
- Price investigation only; no value changed. For A-2027-0101, `$1,385,000` comes from `acquisition_facts.proposed_price`; `buildSf1449` chooses `proposed_price || award_amount`, then uses `faceLine(firstClin, price, commercial)` for blocks 21–24 and `dollars(price)` for block 26. The saved Evaluation of Quotations carries the same amount as `recommended_price`/`quoter_1_price`, and the saved PNM carries it as `negotiated_price`. `$1,450,000` comes from `acquisition_facts.estimated_value`; `packetTransmittal` uses it in `action` (`supports the award … at …`). It matches the IGCE CLIN and acquisition CLIN sums, whose source is `igce_estimate`.
- No database, query, mutation, state/readiness/metric/forecast semantics, field order, citation, method, or unrelated generator/export changed.

# IA Fix R4 — September 25, 2026

- SF 1449 flattened PDF (`src/routes/forms.$formKey.$acquisitionId.tsx`, `src/lib/pdf-out.ts`): its PDF-only header changes U+2212 in `T−` to ASCII `-` in the title and first footer line. No other title, field, value, order, size, gap, font, filename, or text changes. Its two footer lines move from y=30/y=20 to y=50/y=40; the prototype note and page count remain at y=30. The flattened form exporter is the only `renderPdf` caller that passes `footer`; therefore only flattened generated-form PDFs move footer lines. Template-engine PDFs and NF 1858 memorandum PDFs pass no footer and retain their prior placement/bytes.
- Memorandum for Record (`src/lib/nf1858.ts`, document route): only `memorandum-for-record` receives explicit Purpose/Filing completion text. Filing uses the rendered `file_tab`; empty Purpose/Filing values preserve visible contracting-officer fill-in markers after cleanup in preview, Word, and PDF. Other templates call the same paragraph builder without this scoped behavior and remain unchanged. Source confirms “No target award date is recorded.” exists only in `situationScaffold` paragraph 3, drafted only when purpose equals `MFR_SITUATION_PURPOSE`; the default no-purpose “Write a memo to file” path does not draft that scaffold.
- Reporting (`use-operational-display.ts`, `reporting.tsx`): on-screen acquisition-view hold flag, reason, and owner now use the shared operational metric. CSV generation and database view remain unchanged.
- Briefing book (`briefing-book.ts`, file route): the file passes the exact countdown view used by its hero. The HTML status page now prints that view’s figure/mode copy while retaining the phase/clock/target line. There is no briefing-book PDF path; its download remains UTF-8 HTML.
- Command palette (`global-search.tsx`): Arrow keys highlight one acquisition-first flat option order; Enter opens/runs the highlighted option, then falls back to the first acquisition or first command. Click and Tab+Enter remain available.
- No queries, mutations, database objects, state/readiness/metric/forecast semantics, command providers, other generators, other forms, or other exports changed.
...
- Slice 6B corrective: NF 1098 assembly StatusMark no longer paints "Not recorded" green — Missing maps to `var(--atrisk)`, Present/Recorded to `var(--ontrack)`, Not recorded and any other value to `var(--muted-foreground)`; visible words unchanged. NF 1098 table wrapped in `overflow-x-auto`. previewAuthStorage.ts restored byte-identical to a1375255.

## IA Fix R3

- CSS order: the file-page hero's `sm:grid-cols-[auto_minmax(0,1fr)]` conflicted with `min-[1440px]:grid-cols-1` because the generated `sm` rule followed the arbitrary breakpoint. The smaller rule is now `sm:max-[1439px]:grid-cols-[auto_minmax(0,1fr)]`; all `min-[1440px]:` uses were checked, and this was the only same-element/same-property breakpoint pair. The built selector is inside a `(width < 1440px)` query.
- Commands: package-level `sideEffects: false` removed the providers' bare-import registration. `SHELL_COMMAND_PROVIDERS` is now a referenced export passed to `matchCommands`; empty search lists the role/presenter-filtered sidebar pages and current-file command, while typed matching accepts page labels, “Go to …”, and “Open file …” with a 40-command limit. Built JavaScript contains both “Go to ” and “Open file ”.
...
## State-leak generators

- `memo-draft.ts` `situationScaffold`, paragraph 2: raw “The file stood in the Administration phase, clock launched” became the audit-backed “The file stood in the Award phase, HOLD (T−N), on hold because <derived reason> (owner: <derived owner>).” When operational context is unavailable, the phase/clock clause is omitted instead of printing raw columns.
- `memo-draft.ts` `memorandumForRecord`: the opening’s raw `current_phase` became the same derived phase; when operational context is unavailable, the phase clause is omitted. All other situation paragraphs and completion blanks are unchanged.
- `documents.$templateKey.$acquisitionId.tsx` `chromeState` / `draftCtx`: the existing `deriveOverviewAcquisitionState` → `computeMetrics` → `explainWorkReadiness` → `overviewCountdownView` result now supplies phase, readiness, T± line, hold reason, and hold owner to newly drafted preview, Word, and PDF memo text. Saved versions and field values are not rewritten.
- `situation-memo.ts` `situationMemo` and `situation-memo-panel.tsx` `SituationMemoPanel`: raw phase text in the file-page copyable preview became derived phase · readiness · T± and derived hold detail; without operational context it reads “Not recorded.”
- `fpds-filling-sheet.ts` `buildFpdsSheet`: the raw `clock_state === "launched"` gate became an audit-backed `isLaunched` input supplied by the file route. The filling sheet and evidence-pack copy no longer select award-state wording from a stale raw clock.
- `briefing-book.ts` `buildBriefingHtml` was left unchanged: its `currentPhase`, `clockState`, countdown, blocker, and owner are already supplied from the file route’s derived lifecycle. Its other `status` fields are Section K and companion-gate statuses, not acquisition state.
- Internal-only raw hits left unchanged: memo chronology phase-status values classify generated sequence milestones and audit `clock_state` text identifies hold/resume events; `blackout-docx.ts`, `option-exercise-docx.ts`, and `sf-forms.ts` use phase only to select an applicable generator/form; `cor-to-request.ts`, `closeout-panel.tsx`, `confidence.ts`, `successor.ts`, `small-business.ts`, `aging.ts`, `simulate.ts`, `scorecard.functions.ts`, `clause-impact.ts`, and `market-research.server.ts` use state as eligibility, filter, calculation, or structured non-document metadata; file-route raw fields in mutations are audit metadata or write conditions. NEAR/evidence-pack/template rendering preserves immutable saved `field_values`; their `status` values describe documents, clauses, approvals, gates, or assembly rows, not acquisition readiness. Reporting CSV, `rowsToCsv`, the reporting view, and nightly extract are unchanged.
- Verification: A-2026-0090 memo-to-file paragraph 2 reads “The file stood in the Award phase, HOLD (T−N), on hold because <derived reason> (owner: <derived owner>).” A-2027-0101 and A-2027-0102 output changes are limited to derived state words. The 0101 transmittal TBD sentence remains unchanged. SF 1449 prefill for A-2027-0101 and A-2026-0090 remains unchanged.
- Typecheck passes. `previewAuthStorage.ts` is byte-identical to `33578977`.
