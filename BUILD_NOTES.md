...
- Copy only: vehicle totals say estimated value; small-business launched-file counts no longer call the values awards; document, Files, Work Queue, and Executive featured-record displays identify bare `estimated_value` amounts as IGCE. Today and palette results show no `estimated_value` amount, and the file hero has no bare value chip, so they required no text change. Already-labelled estimate/IGCE, threshold, form-field, CSV, and non-`estimated_value` amounts remain unchanged.
