...
- Internal-only raw hits left unchanged: memo chronology phase-status values classify generated sequence milestones and audit `clock_state` text identifies hold/resume events; `option-exercise-docx.ts` and `sf-forms.ts` use phase only to select an applicable generator/form; `cor-to-request.ts`, `closeout-panel.tsx`, `confidence.ts`, `successor.ts`, `small-business.ts`, `aging.ts`, `simulate.ts`, `scorecard.functions.ts`, and `clause-impact.ts` use state as eligibility/filter/calculation conditions; file-route raw fields in mutations are audit metadata or write conditions. NEAR/evidence-pack/template rendering preserves immutable saved `field_values`; their `status` values describe documents, clauses, approvals, gates, or assembly rows, not acquisition readiness. Reporting CSV, `rowsToCsv`, the reporting view, and nightly extract are unchanged.
