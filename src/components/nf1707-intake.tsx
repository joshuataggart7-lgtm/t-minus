...
export function Nf1707Intake({answers,setAnswers,fields,facts,evmThreshold}:{answers:NfAnswers;setAnswers:React.Dispatch<React.SetStateAction<NfAnswers>>;fields:Nf1707Field[];facts:IntakeFacts;evmThreshold:number}){
 ...
 <div className="max-w-[70ch] space-y-3">{sections.map(s=>{const qs=questionsFor(s);const summary=qs.length?sectionSummary(qs,merged):"Not applicable";return <details key={s.key} className="rounded-lg border border-border bg-background" open={false}><summary className="cursor-pointer list-none px-4 py-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="text-[18px] font-medium">Section {s.key} · {s.title}</span><span className="mt-1 block text-[13px] text-muted-foreground">{s.citation}</span>{summary?<span className="mt-1 block text-[13px]">{summary}</span>:null}</summary><div className="divide-y divide-border border-t border-border px-4">{qs.length?qs.map(question=><QuestionRow key={question.key} question={question} answers={merged} setAnswers={setAnswers}/>):<p className="py-4 text-[15px] text-muted-foreground">This section does not apply to this requirement. It exports as Not applicable.</p>}</div></details>})}</div>
